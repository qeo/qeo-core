...
/**
 * Implementation of a state change reader that uses reflection to determine the type. This type of state reader uses a
 * listener for change notifications.
 *
 * @param <TYPE> The class of the Qeo data type.
 * @param <DATA> The type of the Qeo data.
 */
public class StateChangeReaderImpl<TYPE, DATA> extends ReaderBase<TYPE, DATA> implements StateChangeReader<DATA>
{
    /**
     * Create a state change reader.
     *
     ...
     */
    public StateChangeReaderImpl(FactoryHelper factoryHelper, EntityAccessor entityAccessor, IntrospectionUtil<TYPE,
        DATA> introspectionUtil, TYPE typedesc, StateChangeReaderListener<DATA> listener,
                                 PolicyUpdateListener policyListener) throws QeoException
    {
        super(factoryHelper, entityAccessor, introspectionUtil, typedesc, EntityType.STATE_DATA, listener,
            policyListener);
    }
}
