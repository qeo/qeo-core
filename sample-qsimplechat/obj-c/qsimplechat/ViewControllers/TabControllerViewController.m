...
- (void)setupQeoFactory {
    ...
    __weak TabControllerViewController* me = self;
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (YES == [standardUserDefaults boolForKey:@"reg_custom"]) {
            ...
            self.factory = [[QEOFactory alloc] initWithFactoryDelegate:me error:&error];
        } else {
            
            // Trigger Framework registration UI
            self.factory = [[QEOFactory alloc] initWithError:&error];
        }
