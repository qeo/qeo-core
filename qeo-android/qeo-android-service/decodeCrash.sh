...
if [ "${2}" = "debug" ]
then
    VERSION="native-android-debug"
else
    VERSION="native-android-symbols"
fi

SYMBOLS="${DIR}/../qeo-android-service-core/build/download/qeo-java/${VERSION}/lib/${1}"
echo "Using symbols from ${SYMBOLS}"
if [ ! -d "${SYMBOLS}" ]
then
    echo "Symbols directory not found" >&2
    exit 1
fi
