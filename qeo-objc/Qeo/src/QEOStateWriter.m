#import "QEOStateWriter.h"
#import "qeo/log.h"
#import "qeocore/api.h"
#import "QEOFactory.h"
#import "QEOEntity.h"

@interface QEOStateWriter()

@end

@implementation QEOStateWriter
{
    qeocore_writer_t *stateWriter;
}
                       ...
                       error:(NSError **)error
{
    ...
    if (self) {
        ...
        stateWriter = qeocore_writer_open(factory.factory,
        ...
        if (ret != QEO_OK){
            ...
            qeo_log_e("Could not make statewriter (%d)", ret);
            return nil;
        }
    ...
    withError:(NSError **)error
{
    ...
    qeocore_data_t *sample = qeocore_writer_data_new(stateWriter);
    ...
    do {
        ...
        ret = qeocore_writer_write(stateWriter, sample);
        if (ret != QEO_OK){
     ...
     withError:(NSError **)error
{
    ...
    do {
        ...
        if ([object marshallOnlyKeyToData:sample withTypeInfo:super.typeInfo] == NO){
            errorInfo = @"Could not marshal sample";
            ret = QEO_EFAIL;
            break;
        }
        
        ret = qeocore_writer_remove(stateWriter, sample);
        if (ret != QEO_OK){
            errorInfo = @"Could not write sample";
            break;
        }
...
{
    qeo_retcode_t ret = qeocore_writer_policy_update(stateWriter);
...
- (void)dealloc
{
    ...
    qeocore_writer_close(stateWriter);
